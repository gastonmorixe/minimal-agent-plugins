...
- **Parents with all-done children.** Same idea. Marking the last open child `done` auto-promotes the parent; marking a parent `done` cascades open children. You do not need a second call. A canceled sibling blocks auto-promote.
...
Task({action: "done", id: "#<phase>b"})  // parent auto-promotes
```
