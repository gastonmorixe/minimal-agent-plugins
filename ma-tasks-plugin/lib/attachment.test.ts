...
describe("renderAttachmentBody", () => {
  ...
  test("renders top-level tasks as columnar rows", () => {
    ...
    expect(lines[0]).toBe("1  todo      first")
    expect(lines[1]).toBe("2  todo      second")
    expect(lines[2]).toBe("3  todo      third")
  })
  test("indents subtasks under parent", () => {
    ...
    const body = renderAttachmentBody(s.list())
    const lines = body.split("\n")
    expect(lines[0]).toBe("1   todo      parent")
    expect(lines[1]).toBe("  1a  todo      c1")
    expect(lines[2]).toBe("  1b  todo      c2")
    expect(lines[3]).toBe("  1c  todo      c3")
    expect(lines[4]).toBe("2   todo      after")
  })
...
describe("TasksAttachment", () => {
  ...
  test("renders the full <ma::agent::tasks> attachment when tasks exist", () => {
    ...
    expect(text).toContain("1  todo      hello")
