/**
 * Mention overlay style facade.
 *
 * Host injects its palette via `MINIMAL_AGENT_PALETTE`. Handlers call
 * `configureSgr(ctx.env.MINIMAL_AGENT_PALETTE)` so the external plugin stays
 * theme-coherent without importing host modules. Fallback palette keeps
 * standalone tests / previews useful outside the host.
 *
 */
...
export function resolveSgr(raw?: string): Sgr {
  ...
  return {
    ...
    yellow: token("yellow", FALLBACK_SGR.yellow),
    green: token("green", FALLBACK_SGR.green),
    gray: token("gray", FALLBACK_SGR.gray),
...
/** Visible cell width for the mention menu's deliberately 1-cell glyph set. */
...
/** Default SGR for resolved at-token buffer highlights. */
export function mentionTokenSgr(): string {
