// source: ma-llm-openai-plugin/responses/request-body.ts (vendored for OpenCode Go Responses surface; keep in sync)
/**
...
export function buildOpenAIResponsesBody(
  ...
  const extendedVendor = vendor as
    | (typeof vendor & { promptCacheKey?: string; promptCacheRetention?: string })
    | undefined
  const cacheKey =
    (typeof extendedVendor?.promptCacheKey === "string" && extendedVendor.promptCacheKey) ||
    req.metadata?.sessionId ||
    undefined
  if (cacheKey) body.prompt_cache_key = cacheKey
  if (
    typeof extendedVendor?.promptCacheRetention === "string" &&
    extendedVendor.promptCacheRetention
  ) {
    body.prompt_cache_retention = extendedVendor.promptCacheRetention
  }
