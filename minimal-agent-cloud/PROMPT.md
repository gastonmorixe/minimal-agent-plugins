...
Login is real: the CLI authenticates to the backend with the OAuth device grant
(RFC 8628) and stores a bearer token locally. `cloudEnabled` is `false` until you
log in, then the feature flags come from the backend (`me { flags }`). The remote
transport's network methods are still a stub until the WebSocket layer lands; the
auth + flags path is live.

### Tools

- `CloudLogin` — log in via the device grant. Prints a short user code + a
  verification URL to approve in your browser, then completes login and saves the
  token. Backend defaults to `http://localhost:4000/api/auth`; override with
  `MINIMAL_AGENT_CLOUD_URL`.
- `CloudStatus` — report login state, feature flags, the backend URL, and whether
  the remote transport is registered. Calling it ensures the transport is
  registered (idempotent).

### Config (env)

- `MINIMAL_AGENT_CLOUD_URL` — auth base URL (default `http://localhost:4000/api/auth`).
- `MINIMAL_AGENT_CLOUD_GRAPHQL_URL` — GraphQL endpoint (default: derived as the
  origin root `/graphql`).
...
The token is stored at `~/.minimal-agent/cloud-auth.json` (owner-only, outside any
repo).
