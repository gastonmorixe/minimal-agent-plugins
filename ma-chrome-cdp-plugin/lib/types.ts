 ...
 * available. TS types are structural and erased at runtime, so a local mirror
 * lets this plugin type-check standalone (bun test, editor IntelliSense) while
 * staying byte-compatible with the agent's real contract. Mirrors the same
 * stub the ma-fetch-plugin keeps.
 *
 * @module lib/types
 */
