...
- **Performance metrics:** `send Performance.enable {}` then `send Performance.getMetrics {}` returns `{metrics:[{name,value}]}` (JSHeapUsedSize, Nodes, LayoutCount, ScriptDuration, ...).
