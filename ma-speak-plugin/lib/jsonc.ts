...
export function stripJsonc(input: string): string {
  ...
  // or `,]` that appears INSIDE a string value (a CSS selector, a User-Agent,
  // a JS `eval` snippet), silently corrupting user config. Instead we defer
  // emitting a structural comma until we see what follows: a `}` / `]` means
  // it was trailing (drop it); anything else flushes it verbatim.
  ...
  // and keeps error-message line numbers aligned).
  let gap = ""
  ...
  // regardless); flush it so `JSON.parse` reports faithfully instead of us
