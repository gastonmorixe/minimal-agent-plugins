 ...
 *   available, and a plugin may not reach into the agent's modules anyway.
 *   It talks to the host only through the shared `TUIContext` the loader
 *   passes in. TypeScript types are structural and erased at runtime, so a
 *   local mirror lets us type-check this plugin standalone (bun test, editor
 *   IntelliSense) while staying byte-compatible with the agent's real contract.
 *
 * If the agent's contract changes, update this file. The fields we actually
 * use are intentionally narrow: see `TUIContext` (just `trigger`, `cwd`,
 * `packageDir`, `env`, `abort`, stdio) and `TUIResult` (the `tool_result`
 * variant only).
 *
 * @module lib/types
 */
...
export interface ToolTrigger {
  type: "tool"
  /** The tool name as declared in the manifest (`Speak` etc.). */
  name: string
